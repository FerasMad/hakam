...
def load_mean(runs: list[str], split: str) -> dict:
    loaded = [dict(np.load(RUNS_DIR / r / f"{split}_scores.npz", allow_pickle=True)) for r in runs]
    ids = loaded[0]["action_ids"]
    for r, s in zip(runs, loaded):
        if not np.array_equal(s["action_ids"], ids):
            raise ValueError(f"{r}: {split} actions differ from {runs[0]}")
    out = {"action_ids": ids, "matches": loaded[0]["matches"]}
    for key in loaded[0]:
        if key.startswith("prob_"):
            out[key] = np.mean([s[key] for s in loaded], axis=0)
...
def main() -> int:
    ...
    valid = load_mean(args.runs, "valid")
    tasks = [k[len("prob_"):] for k in valid if k.startswith("prob_")]
    report = {"runs": args.runs, "tasks": tasks, "valid": {}, "test": {}, "thresholds": {}}
