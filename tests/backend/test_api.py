...
def test_oversized_upload_is_rejected_and_cleaned(tmp_path, monkeypatch):
    weights = _artifact_directory(tmp_path)
    uploads = tmp_path / "uploads"
    predictor = RecordingPredictor()
    monkeypatch.setenv("HAKAM_WEIGHTS_DIR", str(weights))
    monkeypatch.setenv("HAKAM_TEMP_DIR", str(uploads))
    monkeypatch.setenv("HAKAM_MAX_UPLOAD_BYTES", "5")
    ...
    with TestClient(create_app(loader, lambda _path: None)) as client:
        response = client.post(
            "/api/analyze",
            files=[("video", ("large.mp4", b"123456", "video/mp4"))],
        )
...
def test_health_response_has_request_and_security_headers(tmp_path, monkeypatch):
    weights = _artifact_directory(tmp_path)
    predictor = RecordingPredictor()
    monkeypatch.setenv("HAKAM_WEIGHTS_DIR", str(weights))
